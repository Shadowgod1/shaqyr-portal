...
import { ArrowRight, Sparkles, LayoutDashboard, Wand2 } from 'lucide-react';
      ...
      <div className="hero">
        ...
        <div style={{position:'relative', zIndex:1}}>
          ...
          <div className="hero-cta">
            {user ? (
              <>
                <Link to="/create-event" className="btn btn-primary" style={{fontSize:'1.05rem', padding:'15px 32px'}}>
                  Создать событие <Wand2 size={18} style={{marginLeft:8}}/>
                </Link>
                <Link to="/dashboard" className="btn btn-ghost" style={{fontSize:'1.05rem', padding:'15px 32px'}}>
                  В личный кабинет <LayoutDashboard size={18} style={{marginLeft:8}}/>
                </Link>
              </>
            ) : (
              <>
                <Link to="/register" className="btn btn-primary" style={{fontSize:'1.05rem', padding:'15px 32px'}}>
                  Начать бесплатно <ArrowRight size={18} style={{marginLeft:8}}/>
                </Link>
                <Link to="/login" className="btn btn-ghost" style={{fontSize:'1.05rem', padding:'15px 32px'}}>
                  Войти
                </Link>
              </>
            )}
...
const Step = ({n, title, desc}) => (
  <div className="glass" style={{padding:'36px 28px', textAlign:'center', position:'relative', overflow:'hidden'}}>
    <div style={{
